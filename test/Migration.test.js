...
contract('Migration', ([alice, bob, carol, dev, minter]) => {
    beforeEach(async () => {
        ...
        this.lp4 = await MockIMigrator.new('LPToken', 'LP4', '1000000', this.lp1.address, { from: minter }); 

    });


    it('ItemHelper migration', async () => {
        await expectRevert(this.chef.setItemHelper(this.itemHelper2.address, { from: alice }), 'Ownable: caller is not the owner');
    ...
    it('LP Migration', async () => {
      ...
      await expectRevert(this.chef.setMigrator(this.lp4.address, {from: alice}), 'Ownable: caller is not the owner');
      ...
      assert.equal((await this.lp4.balanceOf(this.chef.address)).toString(), '20');
